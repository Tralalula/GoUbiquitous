...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener {
    ...
    private void updateWatch() {
        if (mGoogleApiClient == null || !mGoogleApiClient.isConnected()) {
            Log.d(LOG_TAG, "UpdateWatch() mGoogleApiClient not connected!");
            return;
        }
        Context context = getContext();
        Log.d(LOG_TAG, "UpdateWatch() mGoogleApiClient is connected!");
        ...
        if (cursor.moveToFirst()) {
            int weatherId = cursor.getInt(INDEX_WEATHER_ID);
            String highTemperature = Utility.formatTemperature(context, cursor.getDouble(INDEX_MAX_TEMP));
            String lowTemperature = Utility.formatTemperature(context, cursor.getDouble(INDEX_MIN_TEMP));

            int iconId = Utility.getIconResourceForWeatherCondition(weatherId);
            Resources resources = context.getResources();

            Bitmap iconBitmap = BitmapFactory.decodeResource(resources, iconId);
            Asset iconAsset = createAssetFromBitmap(iconBitmap);

            PutDataMapRequest dataMapRequest = PutDataMapRequest.create("/weather-data");
            dataMapRequest.getDataMap().putAsset("icon", iconAsset);
            ...
            Log.d(LOG_TAG, "High temperature value: " + highTemperature);
            Log.d(LOG_TAG, "Low temperature value: " + lowTemperature);
        }
        cursor.close();
    }
